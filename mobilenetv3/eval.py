...
def main():
    ...
    assert os.path.exists(class_mapping_path), f"类别映射文件 {class_mapping_path} 不存在"

    with open(class_mapping_path, "r") as f:
        class_mapping = json.load(f)
        class_indict = class_mapping["idx_to_class"]
    # 创建模型
    model = mobilenetv3(n_class=3, mode='large', width_mult=1.0).to(device)

    # 加载模型权重
    model_weight_path = './weights/best_model_weights1.pth'
    ...
    plt.title('Confusion Matrix - MobileNetV3-Large', fontsize=20)  # 图表标题，字体大小设置为 20

    # 设置横坐标和纵坐标的字体大小
    plt.xticks(fontsize=14)  # 设置横坐标字体大小
    plt.yticks(fontsize=14)  # 设置纵坐标字体大小

    plt.savefig("./results/confusion_matrix1.png", bbox_inches='tight')  # 保存为文件
    ...
    plt.title('ROC Curve - MobileNetV3-Large', fontsize=20)
    plt.legend(loc='lower right', fontsize=14)
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    plt.savefig("./results/roc_curve1.png", bbox_inches='tight')  # 保存为文件


    # 保存验证结果
    with open('./results/validation_results1.txt', 'w') as f:
        f.write("模型评估结果：\n")
